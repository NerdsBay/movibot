        ...
        <div class="movie-browse-agile">
            <div class="browse-agile-w3ls general-w3ls">
                <div class="tittle-head">
                    <div class="container">
                        <h4 class="latest-text">Asian Movies</h4>
                        <div class="agileits-single-top">
                            <ol class="breadcrumb">
                                <li><a href="<?php echo site_url; ?>">Home</a></li>
                                <li class="active">Asian Movies</li>
                            </ol>
                        </div>
                    </div>
                </div>
                <div class="container">
					...
					<?php 
						if(!empty($movies)){
							foreach($movies as $movie){	
								echo '<div class="col-md-2 w3l-movie-gride-agile">
									<a href="'.$sys->createLink($sys->parse_name($movie["link"])).'" class="hvr-shutter-out-horizontal list-movies">
										<img src="'.$sys->open($movie["img"]).'" title="album-name" class="img-movie" alt="" />
							...
							echo "<h1>No more asian movies found!</h1>";
						}
					?>
                        <div class="clearfix"></div>
                    </div>
                ...
                <ul>
					<?php
						if(!empty($movies)){
							if(!empty($page)){
								if($page == 2 && $page !== 1){
									echo '<li><a class="last" href="'.site_url.'/asia">Prev</a></li>';
								} else {
									$ppage = $page - 1;
									echo '<li><a class="last" href="'.site_url.'/asia/page/'.$ppage.'">Prev</a></li>';
								}
							}
							if(empty($page)){
								$npage = 2;
								echo '<li><a class="last" href="'.site_url.'/asia/page/'.$npage.'">Next</a></li>';
							} else {
								if($page <= 1){
									$npage = 2;
								} else {
